...
var coornull=-999999,pxmin=360,pxmax=-360,pymin=90,pymax=90;
...
function showSideMap(fname,group,date){
	...
	obliquemap = L.mapbox.map('side', null,{
        ...
        maxBounds: null }).setView([0, 0], obliquemapInitZoom);
...
$(document).ready(function () {
	$('body').layout({  
		 ...
		 east: {size: 0.45, maskObjects: true, resizable: false, showOverflowOnHover: false},
		 north: {size: 52,  maskObjects: false, resizable: false, showOverflowOnHover: true},
		 //south: {size: 20}, //placeholder for footer
		 center: {size: 0.55, maskObjects: true, resizable: false, slidable:  true, showOverflowOnHover: false}, 		
		 onresize: function () {
	...
	map2015 = L.mapbox.map('map2015', undefined, {maxZoom: 14, maxBounds: mapBoundsNA}).addLayer(mbStreet2015).setView(center, initZoom);
    ...
    //map2015.on('zoomend', onZoomend);
     ...
     grids2015.on('click', function(e) {
    		...
    		if (e.data){
    			...
    			polygonOutline = new L.Polygon(polygonPoints,{color: "#043e9a", weight: 4});
